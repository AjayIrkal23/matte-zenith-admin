...
export function ViolationPicker({ isOpen, onClose, onViolationSelected, imageName, violations, startInAdd = false }: ViolationPickerProps) {
        ...
        {isAdding ? (
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="violation-name" className="text-text-primary text-sm font-medium">
                Name
              </Label>
              <Input
                id="violation-name"
            ...
            <div className="space-y-2">
              <Label htmlFor="violation-desc" className="text-text-primary text-sm font-medium">
                Description
              </Label>
              <Textarea
                id="violation-desc"
            ...
            <div className="space-y-2">
              <Label className="text-text-primary text-sm font-medium">Severity</Label>
              <Select value={severity} onValueChange={(v) => setSeverity(v as IViolation["severity"]) }>
                ...
                <SelectContent className="bg-panel-bg border-panel-border">
                  {Object.keys(severityIconMap).map((level) => (
                    <SelectItem key={level} value={level} className="text-text-primary hover:bg-hover-overlay">
                      {level}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button onClick={handleSubmit} disabled={!name || !description} className="btn-adani">
                Add
              </Button>
            </div>
          </div>
          ...
          <>
            ...
            <div className="flex justify-between pt-4">
              <Button variant="ghost" onClick={() => setIsAdding(true)} className="text-adani-primary hover:bg-adani-primary/10">
                Add New Violation
              </Button>
